...
class Grid:
    ...
    def throw_pellet(self, x: int, y: int):
        """
        Throw a pellet at a point (x, y)
        Feeds fish if any, else leaves a pellet

        Args:
            x: x coordinate of point to target
        ...
        if self.at(x, y) in (FISH_CHAR, HAPPY_FISH_CHAR):
            self.replace_point(x, y, HAPPY_FISH_CHAR)
        else:
            self.replace_point(x, y, PELLET_CHAR)
